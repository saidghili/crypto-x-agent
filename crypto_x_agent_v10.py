...
print("VERSION V10 - GPT + SLIDING WINDOW + SQLITE CACHE + PUMP DETECTION + BACKTEST READY")
...
GPT_MAX_CANDIDATES = int(os.getenv("GPT_MAX_CANDIDATES", "12"))
...
TIER_3 = {x.lower() for x in {
    "CryptoMichNL", "CrashiusClay69", "BigCheds", "AltcoinSherpa",
    "Austin_Federa", "ByzGeneral", "Sheldino_D", "LarpVonTrier",
    "TheDeFinvestor", "HopiumPapi", "rektfencer", "RunnerXBT", "MuroCrypto"
}}
...
BLACKLIST_TICKERS = {
    "$USD", "$USDT", "$USDC", "$BTC", "$ETH", "$SPY", "$QQQ", "$DIA",
    "$IWM", "$VIX", "$XAU", "$XAG", "$GOLD", "$SILVER", "$OIL",
    "$ORCL", "$SMCI", "$CPB", "$TSLA", "$AAPL", "$MSFT", "$GOOGL",
    "$GOOG", "$AMZN", "$META", "$NVDA", "$NFLX", "$AMD", "$INTC",
    "$PLTR", "$MSTR", "$COIN", "$HOOD", "$NKE", "$DIS", "$K", "$M",
    "$B", "$A", "$C", "$D", "$I", "$O", "$X", "$Z", "$SPX", "$IXIC",
    "$RUT", "$DXY", "$MU", "$EWY", "$GLD", "$SLV", "$RAVE"
}
KNOWN_LARGE_CAPS = {
    "$SOL", "$BNB", "$XRP", "$DOGE", "$ADA", "$AVAX", "$LINK", "$AAVE",
    "$SUI", "$TRX", "$XMR", "$ZEC", "$NEAR", "$PEPE", "$SHIB", "$FLOKI",
    "$ARB", "$OP", "$INJ", "$APT", "$DOT", "$LTC", "$BCH", "$TON",
    "$UNI", "$MKR", "$RNDR", "$RENDER", "$FET", "$TAO", "$ONDO", "$PENDLE",
    "$SEI", "$TIA", "$JUP", "$WIF", "$BONK"
}
...
def get_cached_market_data(ticker_symbol):
    try:
        conn = sqlite3.connect(CACHE_DB)
        cur = conn.cursor()
        cur.execute("SELECT data, timestamp FROM coingecko_cache WHERE ticker = ?", (ticker_symbol,))
        row = cur.fetchone()
        conn.close()
        if not row:
            return None
...
def get_last_run_time_for_account(account):
    ...
    try:
        conn = sqlite3.connect(CACHE_DB)
        cur = conn.cursor()
        cur.execute("SELECT last_run_time FROM run_metadata WHERE account = ?", (account,))
        row = cur.fetchone()
        conn.close()
...
def cache_tweets(account, tweets):
    ...
    cur.execute(
        """DELETE FROM tweets_cache WHERE account = ?
           AND tweet_id NOT IN (
             SELECT tweet_id FROM tweets_cache WHERE account = ?
...
def is_promotional_tweet(text):
    patterns = [
        r"committing\s+\$", r"launching\s+", r"celebrate", r"excited\s+to\s+announce",
        r"proud\s+to\s+present", r"we\s+(?:are\s+)?committed", r"100k|1m\s+(?:in|incentive)",
...
def analyze_sentiment(text):
    text_lower = (text or "").lower()
    positive_count = sum(1 for w in POSITIVE_KEYWORDS if re.search(rf"\b{re.escape(w)}\b", text_lower))
    negative_count = sum(1 for w in NEGATIVE_KEYWORDS if re.search(rf"\b{re.escape(w)}\b", text_lower))
    positive_count += sum(2 for p in BULLISH_PATTERNS if p in text_lower)
    negative_count += sum(2 for p in BEARISH_PATTERNS if p in text_lower)
    if is_promotional_tweet(text) and negative_count == 0:
        positive_count = max(positive_count, 1)
    if positive_count + negative_count == 0:
        return 0.0
    return max(-1.0, min(1.0, (positive_count - negative_count) / (positive_count + negative_count)))
...
def fetch_latest_tweets(username, limit=20, since_time=None):
    ...
    for t in tweets:
        ...
        if not text or text.strip().lower().startswith("rt @"):
            continue
        tweet = {
            "id": str(tweet_id),
...
def extract_tickers(text):
    ...
    for ticker in raw:
        ticker = ticker.upper()
        if re.fullmatch(r"\$[0-9]+", ticker) or re.fullmatch(r"\$[0-9]+[KMB]", ticker):
            continue
...
def extract_contracts_and_links(text):
    ...
    dex_links = re.findall(r"https?://(?:www\.)?dexscreener\.com/\S+", text)
    pump_links = re.findall(r"https?://(?:www\.)?pump\.fun/\S+", text)
    gecko_links = re.findall(r"https?://(?:www\.)?geckoterminal\.com/\S+", text)
    return evm_contracts, solana_contracts, dex_links, pump_links, gecko_links
...
def check_coingecko_market_data(ticker_symbol):
    ...
    try:
        ...
        r = requests.get("https://api.coingecko.com/api/v3/search", params={"query": ticker_clean}, timeout=5)
        if r.status_code != 200:
            return None
        ...
        r2 = requests.get(f"https://api.coingecko.com/api/v3/coins/{coin_id}", params=market_params, timeout=5)
        if r2.status_code != 200:
            return None
        md = r2.json()
        market = md.get("market_data", {})
        result = {
            "name": md.get("name"),
            ...
            "price_change_24h_percent": market.get("price_change_percentage_24h"),
        }
...
def detect_pump(market_data):
    ...
    if not flags:
        return None
    level = "critical" if pump_score >= 60 else "high" if pump_score >= 30 else "moderate"
    return {"detected": True, "pump_score": pump_score, "pump_level": level, "flags": flags}
...
def score_ticker(ticker, mentions):
    ...
    counts_by_author = Counter(m["author"] for m in mentions)
    risk_penalty_spam = min(15, max(counts_by_author.values()) * 3) if author_count == 1 and counts_by_author else 0
    ...
    if author_count == 1:
        ...
        raw = min(raw, 65 if tier_1_author and has_contract else 45)
...
def classify_signal(ticker, mentions, score, avg_sentiment):
    ...
    if score >= 70 and avg_sentiment > 0.3 and (author_count >= 2 or (has_tier1 and has_contract)):
        return "strong_buy"
    if score >= 60 and avg_sentiment > 0.1 and (author_count >= 2 or has_tier1):
        return "watchlist"
...
def analyze_with_gpt(ticker, signal_data, market_validation, pump_detection):
    if not OPENAI_AVAILABLE or not USE_GPT_SYNTHESIS or not OPENAI_API_KEY:
        return None
    try:
        client = OpenAI(api_key=OPENAI_API_KEY)
        prompt = f"""You are a strict crypto social-signal analyst. Use only the provided data.
Do not invent prices, listings, holders, or on-chain facts.

TICKER: {ticker}
SOCIAL: score={signal_data.get('score')}/100 sentiment={signal_data.get('avg_sentiment')} mentions={signal_data.get('mention_count')} author_count={signal_data.get('author_count')} authors={signal_data.get('authors')}
NARRATIVES: {signal_data.get('narratives')}
MARKET: price={fmt_usd(signal_data.get('current_price_usd'))} market_cap={fmt_usd(signal_data.get('market_cap_usd'))} volume_24h={fmt_usd(signal_data.get('volume_24h_usd'))} change_24h={fmt_pct(signal_data.get('price_change_24h_percent'))}
MARKET_VALIDATION: {market_validation}
PUMP_DETECTION: {pump_detection}

Rules:
- Reject nano caps below $500K market cap.
- Treat zero volume, critical market risk, and critical pump flags as SKIP unless social evidence is exceptional.
- Prefer watchlist over buy-now when data is incomplete.
- This is not financial advice.

Return JSON only with keys: verdict, confidence, reasoning, market_risk, recommendation, key_risks, upside_potential.
Allowed verdict: GEM, RISKY_POTENTIAL, SKIP.
Allowed recommendation: WATCHLIST, WAIT, AVOID.
"""
...
def build_report_data(tweets):
    ...
    for ticker, mentions in ticker_mentions.items():
        ...
        gpt_analysis = None
        if signal_type in {"strong_buy", "watchlist"} and len([r for r in ranked if r.get("gpt_analysis")]) < GPT_MAX_CANDIDATES:
            gpt_analysis = analyze_with_gpt(ticker, signal_data, market_validation, pump_detection)
        ranked.append({
            "ticker": ticker,
            "score": score,
            "avg_sentiment": round(avg_sentiment, 3),
    ...
    return {
        ...
        "weak_signals": [r for r in ranked if r["signal_type"] == "weak_signal"],
        "ranked": ranked,
...
def build_text_report(data):
    lines = []
    lines.append("Crypto X Trend Report V10 - GPT + Sliding Window + Cache + Pump Detection")
    lines.append("=" * 80)
    lines.append(f"Date UTC: {data['generated_at_utc']}")
    lines.append(f"Tweets analyzed: {data['tweets_analyzed']} | new: {data.get('tweets_new', 0)} | cached: {data.get('tweets_from_cache', 0)}")
    lines.append(f"Ignored ticker-list tweets: {data['ignored_list_tweets']}")
    lines.append(f"GPT synthesis: {'enabled' if USE_GPT_SYNTHESIS else 'disabled'} ({GPT_MODEL})")
    lines.append("")
    ...
    if data["narratives"]:
        ...
        lines.append("  None")
    lines.append("")
    sections = [("STRONG BUY", data["strong_buy"], 10), ("WATCHLIST", data["watchlist"], 12), ("RED FLAGS", data["red_flags"], 10)]
    for title, rows, limit in sections:
        ...
        if not rows:
            lines.append("  None")
        for r in rows[:limit]:
            pump = r.get("pump_detection")
            gpt = r.get("gpt_analysis")
            lines.append(f"  {r['ticker']} - score {r['score']}/100 - sentiment {r['avg_sentiment']} - {r['author_count']} accounts")
            lines.append(f"    price {fmt_usd(r.get('current_price_usd'))} | market risk {r['market_validation'].get('risk_level')} | authors {', '.join('@' + a for a in r['authors'][:8])}")
            if pump:
                lines.append(f"    pump: {pump.get('pump_level')} ({pump.get('pump_score')}) flags={', '.join(pump.get('flags', []))}")
            if gpt:
                lines.append(f"    GPT: {gpt.get('verdict')} / {gpt.get('recommendation')} / confidence {gpt.get('confidence')} - {gpt.get('reasoning')}")
            lines.append("")
        lines.append("")
    lines.append("CONTRACTS / LINKS")
    lines.append("-" * 80)
    if data["new_contracts"]:
        for item in data["new_contracts"][:10]:
            ...
            if item["sol_contracts"]:
                lines.append(f"    SOL: {', '.join(item['sol_contracts'][:2])}")
            if item["dex_links"] or item["pump_links"] or item["gecko_links"]:
                lines.append(f"    Links: {', '.join((item['dex_links'] + item['pump_links'] + item['gecko_links'])[:3])}")
            lines.append("")
    else:
        lines.append("  None")
    lines.append("Notes: social signal only, not financial advice. Verify liquidity, holders, contract risk, and execution venue before any trade.")
    return "\n".join(lines)
...
def save_outputs(data, report):
    ...
    txt_path = os.path.join(OUTPUT_DIR, f"crypto_x_report_v10_{stamp}.txt")
    json_path = os.path.join(OUTPUT_DIR, f"crypto_x_report_v10_{stamp}.json")
    csv_path = os.path.join(OUTPUT_DIR, f"crypto_x_ranked_v10_{stamp}.csv")
    ...
    with open(csv_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "ticker", "score", "sentiment", "mentions", "author_count", "authors", "signal_type",
            "price_usd", "market_cap", "volume_24h", "market_risk", "market_flags",
            "pump_detected", "pump_level", "gpt_verdict", "gpt_recommendation", "signal_timestamp_utc"
        ])
        for r in data["ranked"]:
            md = r["market_data"] or {}
            mv = r["market_validation"] or {}
            pump = r["pump_detection"] or {}
            gpt = r["gpt_analysis"] or {}
            writer.writerow([
                r["ticker"], r["score"], r["avg_sentiment"], r["mention_count"], r["author_count"],
                ",".join(r["authors"]), r["signal_type"], r.get("current_price_usd"),
                md.get("market_cap_usd"), md.get("volume_24h_usd"), mv.get("risk_level"),
                ",".join(mv.get("flags", [])), pump.get("detected", False), pump.get("pump_level"),
                gpt.get("verdict"), gpt.get("recommendation"), r["signal_timestamp_utc"],
            ])
...
def main():
    ...
    print(f"Fetch per account: {FETCH_TWEETS_PER_RUN}")
    print(f"Sliding cache per account: {MAX_TWEETS_PER_ACCOUNT}")
    for account in ACCOUNTS_LIST:
        account_norm = norm_author(account)
        print(f"\n@{account}")
        last_run_time = get_last_run_time_for_account(account_norm)
        print(f"  Last successful run: {last_run_time or 'none'}")
        new_tweets, fetch_ok = fetch_latest_tweets(account, limit=FETCH_TWEETS_PER_RUN, since_time=last_run_time)
        print(f"  New tweets fetched: {len(new_tweets)}")
        if fetch_ok:
            cache_tweets(account_norm, new_tweets)
            successful_accounts.append(account_norm)
        else:
            print("  Fetch failed: last_run_time will not be advanced for this account")
        window = deduplicate_tweets(get_cached_tweets(account_norm, limit=MAX_TWEETS_PER_ACCOUNT))
        print(f"  Sliding window tweets used: {len(window)}")
        all_tweets.extend(window)
    all_tweets = deduplicate_tweets(all_tweets)
    print(f"\nTotal tweets after global dedupe: {len(all_tweets)}")
    ...
    if SEND_EMAIL_REPORT:
        send_email("Crypto X Trend Report V10", report)
