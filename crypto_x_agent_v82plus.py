...
# === CACHE FUNCTIONS ===
def init_cache_db():
    """Initialise la base de données sqlite3 pour le caching"""
    try:
...
def get_cached_market_data(ticker_symbol):
    """Récupère les données en cache si disponibles et fraîches"""
    try:
        conn = sqlite3.connect(CACHE_DB)
        cursor = conn.cursor()
    ...
    try:
        ...
        for tweet in tweets:
            cursor.execute("""
                ...
                (account, tweet_id, text, created_at, cached_at)
                VALUES (?, ?, ?, ?, ?)
            """, (account, tweet.get("id"), tweet.get("text"),
                  tweet.get("created_at"), now))
        cursor.execute("""
            DELETE FROM tweets_cache WHERE account = ? AND rowid NOT IN (
                SELECT rowid FROM tweets_cache WHERE account = ?
...
def get_cached_tweets(account, limit=None):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT tweet_id, text, created_at FROM tweets_cache
...
def detect_pump(market_data):
    """Détecte les signaux de pump basés sur volume et volatilité"""
    if not market_data:
        return None
    ...
    # Volume/Market Cap ratio anormal (>200% = suspicious)
    if market_cap > 0:
    ...
    # Volatilité extrême 24h (>100% = pump pattern)
...
def fetch_latest_tweets(username, limit=20, since_time=None):
    ...
    for t in tweets:
        ...
        # Filtrer par since_time si fourni
        if since_time and created_at:
            try:
                tweet_time = datetime.fromisoformat(created_at.replace("Z", "+00:00")).isoformat()
...
def build_text_report(data):
    lines = []
    lines.append("Crypto X Trend Report V8.2+ (Caching + Pump Detection)")
    lines.append("=" * 80)
    ...
    if data["strong_buy"]:
        for r in data["strong_buy"][:10]:
            pump_warning = " ⚠️ PUMP DETECTED" if r["pump_detection"] and r["pump_detection"].get("detected") else ""
    ...
    if data["watchlist"]:
        for r in data["watchlist"][:10]:
            pump_warning = " ⚠️ PUMP" if r["pump_detection"] and r["pump_detection"].get("detected") else ""
            lines.append(f"  {r['ticker']} — Score {r['score']}/100 — {r['author_count']} comptes{pump_warning}")
            lines.append(f"    Sentiment: {r['avg_sentiment']:+.2f} | Risk: {r['market_validation'].get('risk_level', '?')}")
            lines.append("")
    else:
...
def save_outputs(data, report):
    ...
    txt_path = os.path.join(OUTPUT_DIR, f"crypto_x_report_v82plus_{stamp}.txt")
    json_path = os.path.join(OUTPUT_DIR, f"crypto_x_report_v82plus_{stamp}.json")
    csv_path = os.path.join(OUTPUT_DIR, f"crypto_x_ranked_v82plus_{stamp}.csv")
    ...
    with open(csv_path, "w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow([
            ...
            "market_cap", "volume_24h", "market_risk", "pump_detected", "pump_level"
        ])
        for r in data["ranked"]:
            ...
            writer.writerow([
                ...
                market_cap, volume, market_risk, pump_detected, pump_level
            ])
...
def main():
    ...
    for account in ACCOUNTS_LIST:
        ...
        sliding_window_tweets = get_cached_tweets(account, limit=MAX_TWEETS_PER_ACCOUNT)
        sliding_window_tweets = deduplicate_tweets(sliding_window_tweets)
        print(f"  Sliding window tweets used for analysis: {len(sliding_window_tweets)}")

        all_tweets.extend(sliding_window_tweets)
